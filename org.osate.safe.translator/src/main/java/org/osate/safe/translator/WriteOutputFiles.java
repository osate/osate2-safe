...
public class WriteOutputFiles {
	...
	public static void writeHazardReport(String reportText, String reportDirectory, String reportName, OutputFormat fmt,
	...
	public static void writeFiles(HashMap<String, String> compsigs, HashMap<String, String> javaClasses, String appName,
		...
		for (String fileName : javaClasses.keySet()) {
			writeStrToFile(javaClasses.get(fileName), appDevDirectory + appName + "/" + fileName + ".java");
		}

		for (String fileName : compsigs.keySet()) {
			writeStrToFile(compsigs.get(fileName), appDevDirectory + appName + "/appcomp/" + fileName + ".compsig.xml");
		}

		writeStrToFile(appSpecContents, appDevDirectory + appName + "/appcfg/" + appName + ".cfg.xml");

	}

	public static void writeDeviceFiles(String superType, String userAPI, String compsigs, String devName,
			String devDirectory) {
		...
		try {
			...
			fw = new FileWriter(devDirectory + devName + "/" + devName + ".compsig.xml");
			fw.write(compsigs);
			fw.close();
